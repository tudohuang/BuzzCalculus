(function () {
  ...
  // 新手保護期：頭幾局不倒數、不計分、提示不扣分。
  //
  // 走查初學者流程的結果：第一次進來的人，第一題就要在 38 秒內答完
  // d/dx(e^x ln x)，倒數對他只有壓力沒有意義。「來不及」這個訊號在
  // 已經會做的人身上才有價值；還在認識題型的人要的是把題目看完。
  // 高中先修給 3 局，其他人給 1 局；大考、生存、Boss、對戰這種
  // 本身就是計時規則的模式不在保護範圍。
  ...
  function gentleStartActive(records) {
    const played = (records.history || []).filter((item) => item.mode !== "placement").length;
    return played < gentleStartQuota(records);
  }
  ...
  function renderTodayCard(records) {
    ...
    if (gentleStartActive(records)) {
      const first = gentleTrim(filled.problems, records);
      const played = (records.history || []).filter((item) => item.mode !== "placement").length;
  ...
  function startPlannedSession(lengthKey) {
    ...
    selectedMode = "quick";
    selectedTopic = "all";
    startQuiz(gentleStartActive(records) ? gentleTrim(plan.filled.problems, records) : plan.filled.problems, { modeKey: "quick" });
  }
  ...
  function renderPlacementNextStep() {
    const records = loadRecords();
    const plan = plannedSession(records);
    const gentle = gentleStartActive(records);
    const count = plan ? (gentle ? gentleTrim(plan.filled.problems, records).length : plan.filled.problems.length) : 12;
  ...
  // 一個「自己抽 10 題來印」的頁面要維護整套列印樣式，價值不夠。
  ...
  function bindProofLangEvents() {
    ...
    textarea.addEventListener("input", () => {
      ...
      proofLangTimer = window.setTimeout(() => {
        proofLangTimer = null;
        refresh();
      }, 260);
    });
  ...
  function insertProofLangTemplate(text) {
    const textarea = app.querySelector("[data-proof-lang-text]");
    if (!textarea) return;
    const value = textarea.value;
    const start = typeof textarea.selectionStart === "number" ? textarea.selectionStart : value.length;
    // 範本永遠自成一行：游標前面沒有換行就先補一個
    ...
    textarea.focus();
    try { textarea.setSelectionRange(caret, caret); } catch (_error) { /* 舊瀏覽器 */ }
  ...
  function renderProofOrderDrill(proof, progress) {
    ...
    // 池子按打亂後的順序**固定不動**：點掉一個，它原地變成打勾的已選，
    // 其他選項一格都不移。第一版是點一下就把剩下的重新排 —— 使用者
    // 瞄準第二步的那一下，點到的已經是滑上來的別句（實測直接把作答弄錯）。
    // 位置穩定是點擊介面的底線。
    return `
      <div class="proof-order-drill">
        ...
        <div class="proof-order-pool">
          ${drill.order.map((stepIndex) => {
            const pickedAt = drill.picked.indexOf(stepIndex);
            return pickedAt >= 0
              ? `<button class="proof-order-step is-taken" disabled>
                  <span class="proof-order-taken-no">${pickedAt + 1}</span>${escapeHtml(steps[stepIndex].text)}
                </button>`
  ...
  function renderQuiz() {
    ...
    // 計算紙暫停中：時間凍結在暫停那一刻，畫面也要顯示凍結的數字
    const frozenNow = !quiz.examMode && quiz.boardPausedAt ? quiz.boardPausedAt : Date.now();
    const elapsed = Math.max(0, Math.floor((frozenNow - quiz.questionStartedAt) / 1000));
  ...
  // 參考答案的渲染版。全站的數學都走 KaTeX，唯獨參考答案一直是
  // pi^4/90、sqrt(2)/2 這種生字串 —— 在回饋卡、結算回顧、錯題本
  ...
  function referenceAnswerHTML(problem) {
    ...
    // 參考答案跟選項一樣做常數摺疊：4·3·x^(3−1) 這種模板原形在初學者眼裡是壞掉的。
    const tex = answerToTex(simplifyChoiceLabel(raw), problem) || answerToTex(raw, problem);
    if (!tex) return escapeHtml(raw);
    return `<span class="math-inline" data-tex="${escapeAttr(tex)}">${renderLiteTex(tex, false)}</span>`;
  }
  ...
  function renderExitModal() {
    ...
    // 一題都沒作答的離開不是「結算」——不記成績、不進錯題本、不動能力分數。
    // 講清楚，使用者才敢放心點進來看一眼再走。
    const untouched = Boolean(quiz && quiz.answers.length === 0 && !hasAnyExamDraft(quiz));
    return `
      <div class="modal-backdrop" data-action="close-modal">
  ...
  function keyIdeaFor(problem) {
    ...
    try {
      // 題目自己的技巧 tag 比技能節點的名字準：切線斜率那題掛在
      // 「曲線分析與中值定理」節點下，但它考的是切線＋鏈鎖律，跟中值定理無關。
      // 有中文名的技巧 tag 先用；沒有才退回技能節點。
      const ownTags = (problem.tags || [])
        .filter((tag) => !META_ANALYSIS_TAGS.has(tag) && TAG_LABELS[tag] && /[一-鿿]/.test(TAG_LABELS[tag]))
        .map((tag) => TAG_LABELS[tag])
        .filter((label, index, list) => list.indexOf(label) === index)
        .slice(0, 2);
      const names = ownTags.length
        ? ownTags
        : window.BuzzSkillGraph.skillsForProblem(problem)
  ...
  function bindEvents() {
    ...
    app.querySelectorAll("[data-ws-field]").forEach((node) => {
      ...
      node.addEventListener("focus", () => {
        if (quiz) quiz.wsFocusKey = node.dataset.wsField;
      });
    });

    // 作圖表的符號 chips：把 (-inf、U 這些難打的記號插進最後聚焦的格子。
    // 手機系統鍵盤要打七次 (-inf, ...) U (...) 是實測級的折磨。
    app.querySelectorAll("[data-ws-insert]").forEach((button) => {
      button.addEventListener("click", () => {
        const current = getCurrentProblem();
        if (!quiz || !current) return;
        const inputs = [...app.querySelectorAll("[data-ws-field]")];
        const target = inputs.find((node) => node.dataset.wsField === quiz.wsFocusKey) || inputs[0];
        if (!target || target.disabled) return;
        const token = button.dataset.wsInsert;
        const start = target.selectionStart ?? target.value.length;
        const end = target.selectionEnd ?? target.value.length;
        target.value = target.value.slice(0, start) + token + target.value.slice(end);
        const cursor = start + token.length;
        target.focus();
        try { target.setSelectionRange(cursor, cursor); } catch (_error) { /* type=number 之類 */ }
        worksheetDraft(current)[target.dataset.wsField] = target.value;
        quiz.draft = serializeWorksheet(current);
      });
    });
  ...
  function handleAction(event) {
    ...
    if (action === "dismiss-update") {
      try { sessionStorage.setItem("buzzcalculus.update-dismissed", "1"); } catch (_error) { /* 私密模式 */ }
      pendingUpdate = null;
      render();
    }
  ...
  function selectPathNodePool(node) {
    ...
    // 新手保護期的主線：由淺入深，不洗牌。第一關第一題本來抽到
    // lim (sin x − x)/x³ —— 要 Taylor 或三次 L'Hôpital —— 而關卡簡介才說
    // 「sin x / x 是核心」。適性排序對還沒有資料的人等於隨機；這時候 rank 就是最好的順序。
    const mixed = gentleStartActive(records)
      ? drawn.map((problem, index) => ({ problem, index })).sort((a, b) => (problemRank(a.problem) - problemRank(b.problem)) || (a.index - b.index)).map((entry) => entry.problem)
      : shuffle(drawn, seedFromString(`${node.id}-mix-${drawn.length}`));
    return padPool(mixed, source, mode.count, { records });
  }
  ...
  function placementRankPools(records = loadRecords()) {
    ...
    for (let rank = 1; rank <= 6; rank += 1) {
      pools[rank] = preferFreshProblems(
        shuffle(
          source.filter((problem) => problemRank(problem) === rank),
  ...
  function placementStartRank(records) {
    return PLACEMENT_START_RANK_BY_CONTEXT[records.onboardingContext || ""] || PLACEMENT_START_RANK;
  }
  ...
  function startPlacementQuiz() {
    ...
    const startRank = placementStartRank(records);
    const used = new Set();
    const lineup = [];
    // 只有第一題是真的起手題；後面的槽位是佔位，作答後由調適邏輯換掉。
    while (lineup.length < PLACEMENT_COUNT) {
      const pick = drawPlacementProblem(pools, startRank, used, lineup.length ? lineup[lineup.length - 1].topic : "");
      if (!pick) break;
      used.add(pick.id);
      lineup.push(pick);
    }
    ...
    startQuiz(lineup, {
      modeKey: "placement",
      answerMode: "choice",
      placement: { rank: startRank, pools }
    });
  }
  ...
  function computePlacementResult(answers) {
    ...
    const unlearned = (answers || []).filter((answer) => answer.reason === "Unlearned").length;
    return { rank, correct, weakTag, unlearned, total: (answers || []).length };
  }
  ...
  function renderUpdateBanner() {
    ...
    // 按過「稍後」這個工作階段就不再出現。實測它跨頁面陰魂不散，
    // 手機上正好蓋住題庫任務板與設定的按鈕 —— 提醒一次是服務，
    // 追著人跑是騷擾。下次重新開站再提醒一次就好。
  ...
  function installApp() {
    // iOS Safari 沒有 beforeinstallprompt —— 按了毫無反應看起來像壞掉的
    // 按鈕（實測回報）。給不了系統對話框，至少給路：分享 → 加入主畫面。
  ...
  function startQuiz(customProblems, options = {}) {
    ...
    if (
      gentleStartActive(records)
      && !quiz.examMode && !quiz.placement && !quiz.pathGate && !quiz.duel && !quiz.namedExam
      && !quiz.survival && !quiz.suddenDeath && !quiz.pressureMode && !options.dailyOne
    ) {
      quiz.practice = true;
      quiz.gentle = true;
    }
    // 定位測驗是診斷，不是比賽：不計分、不給提示（提示會讓定位失真）、
    // 答錯不停下來說教，直接下一題。
    if (quiz.placement) {
      quiz.noHint = true;
      // 新手保護期的定位連倒數都不要：先量「會不會」就好。
      if (gentleStartActive(records)) quiz.noTimer = true;
    }
    view = "quiz";
    lastVisibilityStamp = Date.now();
  ...
  function questionTimeLimit(currentQuiz, problem) {
    ...
    // 一個還在讀題的人被 24 秒催掉，測到的是緊張不是程度。
    if (currentQuiz && currentQuiz.placement) return Math.max(60, base * 2);
  ...
  function adaptiveShuffle(pool, records, seed) {
    ...
    const ordered = pool
      ...
      .map((item) => item.problem);
    return interleaveBySkill(spreadFamilies(ordered));
  }

  // 同一個模板家族（tmpl-lim-log-ratio-001/002/003…）在一局裡重複出現
  // 等於同一題換數字出三次 —— 實測 19 題裡 log(1+ax)/log(1+bx) 出了 3 次。
  // 把同家族的後續成員推遲到排序尾端：前段（實際會被抽走的那一段）
  // 每個家族最多一題，池子小到不夠用時它們才會回來。
  function problemFamily(problem) {
    const match = /^(tmpl-[a-z-]+)-\d+$/.exec(problem.id || "");
    return match ? match[1] : null;
  }

  function spreadFamilies(ordered) {
    const seen = new Set();
    const head = [];
    const deferred = [];
    ordered.forEach((problem) => {
  ...
  function recordAnswer({ status, reason, input, detail }) {
    if (!quiz) return;
    if (quiz.feedback) return;
    // 計算紙開著（倒數暫停中）直接送出：先把暫停的時間補回去，
    // elapsed 才不會把寫算的時間算成作答時間。
    if (quiz.boardPausedAt) resumeQuestionTimer();
    const problem = getCurrentProblem();
    ...
    //
    // 作圖表是例外：介面承諾「送出後會顯示正確的圖讓你對照」，
    // 而 950ms 的自動前進讓那張圖根本來不及出現（實測：答對直接跳結算，
    // 從頭到尾沒有對照圖）。作圖表答完一律停下來，看完自己按下一題。
    const pauseForComparison = getCurrentProblem()?.answerKind === "worksheet";
    if ((correct && !pauseForComparison) || quiz.examMode || quiz.placement) {
    ...
    if ((!correct || pauseForComparison) && !quiz.examMode && !quiz.placement) scrollFeedbackIntoView();
  }

  // 計算紙暫停：非考試的計時題，攤開計算紙就停表，收起來把暫停的
  // 時間補回 questionStartedAt。大考整份倒數照實走（續局卡也是這樣承諾的）。
  function pauseQuestionTimer() {
    if (!quiz || quiz.examMode || quiz.noTimer || quiz.practice || quiz.boardPausedAt) return;
    quiz.boardPausedAt = Date.now();
    stopTicker();
  }

  function resumeQuestionTimer() {
    if (!quiz || !quiz.boardPausedAt) return;
    quiz.questionStartedAt += Date.now() - quiz.boardPausedAt;
    quiz.boardPausedAt = null;
    startTicker();
  }
  ...
  function advanceToNextQuestion() {
    ...
    quiz.boardOpen = false;
    quiz.boardFullscreen = false;
    quiz.boardPausedAt = null; // 新題目重新計時，暫停狀態不能帶過去
  ...
  function finishQuiz() {
    stopTicker();
    // 一題都還沒作答就離開 = 誤入，不是成績。
    // 實測慘案：點進 Integral Bee 看一眼就離開，被記 0/10 未及格、
    // 10 題沒看過的題全部灌進錯題本排今天到期、能力分數 89→72。
    // 「看了一眼」不該在任何資料上留下痕跡。（時間到的收卷不在此列 ——
    // 但整份時間走完仍一題沒碰，也一樣當作沒來過。）
    if (quiz && quiz.answers.length === 0 && !hasAnyExamDraft(quiz)) {
      clearActiveSession();
      quiz = null;
      view = "home";
  ...
  // 大考裡有沒有任何一題寫了草稿？有的話收卷要照算（寫了就該算）。
  function hasAnyExamDraft(currentQuiz) {
    if (!currentQuiz || !currentQuiz.examMode) return false;
    const drafts = { ...(currentQuiz.draftMap || {}) };
    const current = currentQuiz.problems[currentQuiz.index];
    if (current && String(currentQuiz.draft || "").trim()) return true;
  ...
  function hintsFor(problem) {
    // 作者寫的提示優先，但罐頭句（「Identify the dominant tool before computing.」）
    // 不算 —— 那是對整包都成立、也就是對這一題沒用的句子，卻要扣分。
    // 沒有作者提示時，先用系統從題目算出來的事實（「最外層是兩個函數相乘」），
    // 最後才是主題的泛用提示。
    const hints = authoredHints(problem).slice(0, 3);
    if (hints.length >= 2) return hints;
    const derived = window.BuzzDerivedHints ? window.BuzzDerivedHints.textFor(problem.id) || "" : "";
    if (derived && !hints.includes(derived)) hints.push(derived);
  ...
  // 誘答至少要是一個算得出來的式子。字串手術（拔鏈鎖係數、除以變數）
  // 偶爾會做出 *y+y*cos(y)、(…)/x,y,z 這種東西 —— 初學者看到會以為網站壞了。
  function wellFormedDistractor(problem, value) {
    ...
    if (depth !== 0) return false;
    if (/^(dne|inf|-inf)$/i.test(text)) return true;
    const probe = evaluateExpression(text, { x: 0.7, y: 0.6, z: 0.8, t: 0.5, n: 3, u: 0.9, a: 0.4, b: 0.3, k: 2 });
    return Number.isFinite(probe);
  }

  // 選項顯示前的常數摺疊。
  //
  // 為什麼在顯示層做而不改題庫：答案字串是判分與驗算的鍵，動它要重跑
  // 四支產生器；而這裡要解決的只是「長相洩題」——同一個值，換一件衣服。
  //   5*3/(3-1)        -> 15/2
  //   4^2/2            -> 8
  //   (3x+1)^(5+1)/(3*(5+1)) -> (3x+1)^6/18（含變數只折純數字括號）
  //   pi/(2*sqrt(2))   -> 原樣（無理數不硬轉小數）
  function foldNumericParens(text) {
    let out = String(text);
    // 4*3*x^(3-1) 這種模板形狀：先把純整數的乘積摺成一個數（4*3 → 12），
    // 但 2*3^2 不能摺成 6^2 —— 第二個數後面接 ^ 就放過。
    for (let round = 0; round < 6; round += 1) {
      const next = out.replace(/(^|[^\w.^])(\d+)\*(\d+)(?![\d.]|\^)/g, (whole, lead, a, b) => `${lead}${Number(a) * Number(b)}`);
      if (next === out) break;
      out = next;
    }
    // 係數 1 不用寫：1*x*y → x*y（微擾誘答把 2 改成 1 時會留下這種形狀）
    out = out.replace(/(^|[^\w.^])1\*(?=[a-z(])/g, "$1");
    for (let round = 0; round < 6; round += 1) {
      const next = out.replace(/\((-?\d+(?:\.\d+)?(?:[+\-*/^]-?\d+(?:\.\d+)?)+)\)/g, (whole, expr) => {
        const value = evaluateExpression(expr, {});
        if (!Number.isFinite(value)) return whole;
        const rounded = Math.round(value);
        if (Math.abs(value - rounded) > 1e-9) return whole; // 只折整數，別把 (1/3) 折成 0.333
  ...
  function approximateFraction(value, maxDenominator) {
    let p0 = 0, q0 = 1, p1 = 1, q1 = 0, x = value;
    ...
    if (!q1) return null;
    return { p: value < 0 ? p1 * Math.sign(value * p1 >= 0 ? 1 : -1) : p1, q: q1, ok: Math.abs(p1 / q1 - value) <= 1e-9 };
  }

  function simplifyChoiceLabel(rawValue) {
    const text = String(rawValue || "").trim();
    if (!text) return text;
    const withoutNames = text.replace(/pi|exp|log|sin|cos|tan|sqrt|sinh|cosh|tanh|asin|acos|atan|abs|DNE|inf/gi, "");
    if (/[A-Za-z]/.test(withoutNames)) return foldNumericParens(text); // 含變數
    if (!/[+\-*/^]/.test(text.slice(1))) return text; // 已是單一數字
    const value = evaluateExpression(text, {});
    if (!Number.isFinite(value)) return foldNumericParens(text);
    const rounded = Math.round(value);
    if (Math.abs(value - rounded) <= 1e-9 && Math.abs(rounded) <= 1e9) return String(rounded);
    const frac = approximateFraction(Math.abs(value), 3600);
    if (frac && frac.ok) return `${value < 0 ? "-" : ""}${frac.p}/${frac.q}`;
    return foldNumericParens(text); // 帶 pi / sqrt 的無理數：保留精確形
  }
  ...
  function* buildChoiceDistractors(problem, correct) {
    ...
    // 格式也要同族：正解是精確形（15/2、pi/4）時，0.5772156649 這種
    // 長小數一看就是別題漂來的（那還真的是 Euler γ）。
    const exactStyle = !/\.\d{4,}/.test(String(correct));
    // 別題的答案只能當**同形**的誘答：同主題、同一組變數、長度相近。
    // 走查初學者流程抓到的：d/dx(4x³) 的選項是 (log x)^(log x)·…、2xy + y cos(xy)、
    // cos(sin(sin x))·… —— 從不相干的題漂來的外星人，靠排除法零秒答對，
    // 還讓產品看起來是壞的。
    const variableSet = (value) => String(value).replace(/pi|exp|log|sin|cos|tan|sqrt|sinh|cosh|tanh|asin|acos|atan|abs|DNE|inf/gi, "").replace(/[^a-z]/gi, "").split("").sort().join("");
    const ownVars = variableSet(correct);
    const ownLength = String(simplifyChoiceLabel(correct)).length;
    const sameShape = (value) => {
      const text = String(value);
      if (variableSet(text) !== ownVars) return false;
      const length = String(simplifyChoiceLabel(text)).length;
      return length <= Math.max(6, ownLength * 1.8) && length >= Math.min(ownLength * 0.4, ownLength);
    };
  ...
  function numericallyPlausibleDistractor(correct, candidate) {
    ...
    if (Math.abs(value) > scale * MAX_DISTRACTOR_RATIO) return false;
    // 反向也要擋：正解是 13 位數時，4 和 22/5 這種選項一眼就知道是湊數 ——
    // 「只有一個大數」等於把答案圈出來（實測 19 階導數那題就這樣被猜對）。
    if (Math.abs(target) > MAX_DISTRACTOR_RATIO && Math.abs(value) < Math.abs(target) / MAX_DISTRACTOR_RATIO) return false;
    return true;
  }
  ...
  // 一串小數在四個精確形的選項裡一眼就是「湊數的那個」。
  function formatChoiceNumber(value) {
    if (!Number.isFinite(value)) return "";
    const rounded = Math.round(value * 1000000) / 1000000;
    if (Number.isInteger(rounded)) return String(rounded);
    const frac = approximateFraction(Math.abs(value), 1000);
    if (frac && frac.ok && frac.q > 1) return `${value < 0 ? "-" : ""}${frac.p}/${frac.q}`;
  ...
  // 結算的分母。中途離開的一局只結算已經答的題 —— 做 2 題就退出被記成
  // 0/13、0%、「先重建基礎」，那是把「沒做」算成「做錯」。
  // 大考（收卷時未答的題會補成未作答）與主線關卡（過關線是整關的題數）照舊。
  function settledTotal(currentQuiz) {
    if (!currentQuiz) return 0;
    if (currentQuiz.examMode || currentQuiz.pathNodeId || currentQuiz.pathGate) return currentQuiz.problems.length;
    return Math.max(currentQuiz.answers.length, 0) || currentQuiz.problems.length;
  }
  ...
  function saveQuizRecord(currentQuiz) {
    ...
    const total = settledTotal(currentQuiz);
    const accuracy = total ? Math.round((correct / total) * 100) : 0;
    ...
    // 定位是診斷：答錯／沒學過都不進錯題本，不然第一份「複習」是一堆沒學過的東西。
    const answerContext = { mistakesMode: currentQuiz.mode === "mistakes", diagnostic: Boolean(currentQuiz.placement) };
    ...
    if (currentQuiz.mode === "placement") {
      ...
      // 定位的結果要真的餵給抽題。難度上限預設 3，而 planner 的每一格
      // 都吃這個值 —— 不同步的話，測出 R6 的人下一局照樣拿到一整份
      // R1–R2 暖身題（實測：19 題裡 17 題送分）。只升不降：
      // 手動調低過上限的人，他的選擇優先。
      const capNow = Number(records.settings?.difficultyCap || 3);
      if (placement.rank > capNow) {
        records.settings = { ...(records.settings || {}), difficultyCap: placement.rank };
      }
  ...
  function updateAnswerRecords(records, answer, finishedAt, context = {}) {
    ...
    if (!answer.correct && !context.diagnostic && answer.reason !== "Unlearned") {
  ...
  function dailyMissionInfo(records, daily) {
    ...
    // 「每日 N 題」按**今天實際答了幾題**算，不限模式。
    // 原本只有「每日挑戰」那個特定模式算數 —— 實測做完定位測驗＋
    // 首訓 27 題，首頁還是「每日 0/12」，讀起來像練的都白練。
    // 目標是讓人每天動手，不是讓人點某顆特定按鈕。
    // （連勝的判定不變：完成每日挑戰那組才續 streak。）
    const today = new Date().toISOString().slice(0, 10);
    let answeredToday = 0;
    (records.history || []).forEach((session) => {
      if (String(session && session.finishedAt || "").slice(0, 10) !== today) return;
      const n = Array.isArray(session.answers) ? session.answers.length : Number(session.total || 0);
      answeredToday += Math.max(0, n);
    });
    const completed = Math.min(target, Math.max(answeredToday, daily ? Number(daily.completed || 0) : 0));
    return {
      target,
      completed,
      done: Boolean(daily) || completed >= target,
  ...
  function buildWeaknessAnalysis(records) {
    ...
    Object.values(records.mistakes || {}).forEach((item) => {
      ...
      // meta tag（rank-1、beginner-friendly、出處）不是技巧，「新手友善 · 6」對弱點分析沒有意義
      (problem.tags || []).forEach((tag) => {
        if (META_ANALYSIS_TAGS.has(tag)) return;
  ...
  // 這題是不是收斂判定題？text 題有兩族：收斂判定（中文詞）與
  // 判型／概念題（答案是英文技巧名）。placeholder 寫錯族，使用者
  // 會照著錯的格式打 —— 實測 Bessel 題掛著「收斂/發散」的提示，
  // 正解卻要打 second order ODE，三層資訊互相矛盾。
  function isConvergenceText(problem) {
    return (problem.answers || []).some((a) => /收斂|發散|converge|diverge/i.test(a));
  }
  ...
  function answerExamples(problem) {
    // 收斂題以外的 text 題不給罐頭 chips —— 「convergent / divergent」
    // 放在判型題旁邊只會把人帶去錯的答案空間。
    if (problem.answerKind === "text") return isConvergenceText(problem) ? ["convergent", "divergent", "conditional", "DNE"] : [];
  ...
  function texAtom(expr) {
    ...
    if (call) {
      ...
      const names = {
        ...
        // 雙曲函數沒對應到指令時，KaTeX 會把 tanh 當四個變數相乘
        // 渲染成斜體 t·a·n·h（實測預覽抓到）
        sinh: `\\sinh\\left(${body}\\right)`,
        cosh: `\\cosh\\left(${body}\\right)`,
        tanh: `\\tanh\\left(${body}\\right)`
      };
  ...
  if (window.__BUZZ_TEST_HOOKS__) {
    window.__BUZZ_TEST_HOOKS__.api = {
      ...
      skillRefreshDue,
      simplifyChoiceLabel,
      buildChoiceDistractors,
      wellFormedDistractor,
      getChoiceOptions,
      hintsFor,
      gentleStartActive,
      numericallyPlausibleDistractor,
      spreadFamilies,
      dailyMissionInfo,
      placeholderFor,
      preferFreshProblems,
