...
/* ═══════════════════════════════════════════════════════════════════
   作圖題（sk-，40 題，2026-09-21）：格子是空的，學生自己把 f 畫上去。
   answerKind "sketch"：sketch.expr 是判分用的曲線，pieces 是必須畫到的 x 區段
   （垂直漸近線兩側分開），answer 存 f 的 LaTeX 給參考答案顯示。
   判分（BuzzGraphSketch.check）看四件事：是不是函數、有沒有畫滿、每一段增減對不對、
   位置在不在容差內；驗算器對「題幹的 f」與「sketch.expr」是否同一個函數。
   跟前面兩段放同一個檔：script 標籤預算滿了。
   ═══════════════════════════════════════════════════════════════════ */
(function () {
  "use strict";

  const SOURCE = "Buzz 作圖題 2026-09";
  const problems = [];
  ...
  function add(problem) {
    ...
    if (problem.rank >= 5) tags.push("boss-rank");
    if (problem.rank <= 2) tags.push("beginner-friendly");
    problems.push({ source: SOURCE, difficulty: Math.min(4, problem.rank), topic: "derivatives", answerKind: "sketch", ...problem, tags });
  }

  // 題幹一律「畫出 f(x)=… 的圖形」；驗算器從 f(x)= 到 \text 之間讀 f。
  const sk = (id, rank, tex, expr, window, pieces, extra) => add({
    id, rank, prompt: `\\text{畫出 }f(x)=${tex}\\text{ 的圖形}`, answer: tex,
    graph: { window }, sketch: { expr, pieces }, timeLimit: extra.timeLimit || 150, tags: extra.tags || [],
    hints: extra.hints, solution: extra.solution
  });

  /* ── A. 多項式（R2–R3）：零點、頂點、極值、反曲點 ── */
  sk("sk-001", 2, "x^2-2x", "x**2-2*x", [-2, 4, -2, 4], [[-1, 3]], {
    tags: ["extrema"], hints: ["零點：x=0 與 x=2。", "頂點在兩零點中間 x=1，f(1)=−1。"],
    solution: "開口向上的拋物線，零點 0、2，頂點 (1,−1)。" });
  sk("sk-002", 2, "4-x^2", "4-x**2", [-3, 3, -6, 5], [[-2.5, 2.5]], {
    tags: ["extrema"], hints: ["開口向下。", "零點 ±2，頂點 (0,4)。"],
    solution: "開口向下的拋物線，頂點 (0,4)，零點 ±2，往兩邊掉下去。" });
  sk("sk-003", 2, "(x-2)^2-1", "(x-2)**2-1", [-1, 5, -2, 4], [[0, 4]], {
    tags: ["extrema"], hints: ["頂點 (2,−1)。", "零點：(x−2)²=1 → x=1, 3。"],
    solution: "把 x² 右移 2、下移 1：頂點 (2,−1)，零點 1 與 3，f(0)=3。" });
  sk("sk-004", 2, "|x|", "abs(x)", [-3, 3, -1, 3], [[-2.5, 2.5]], {
    tags: ["absolute-value"], hints: ["x≥0 時是 y=x。", "x<0 時是 y=−x：兩條直線在原點交成 V。"],
    solution: "V 形：原點是尖點，兩側斜率 ±1。" });
  sk("sk-005", 2, "|x-1|-1", "abs(x-1)-1", [-2, 4, -2, 3], [[-1, 3]], {
    tags: ["absolute-value"], hints: ["|x| 右移 1、下移 1。", "尖點在 (1,−1)，零點 0 與 2。"],
    solution: "V 形，尖點 (1,−1)，穿過 (0,0) 與 (2,0)。" });
  sk("sk-006", 2, "x^3", "x**3", [-2, 2, -4, 4], [[-1.5, 1.5]], {
    tags: ["inflection"], hints: ["處處遞增。", "原點是反曲點：左邊凹向下、右邊凹向上。"],
    solution: "遞增，原點附近很平（f′(0)=0 但不是極值），左凹下右凹上。" });
  sk("sk-007", 2, "x^3+1", "x**3+1", [-2, 2, -4, 4], [[-1.6, 1.4]], {
    tags: ["inflection"], hints: ["x³ 上移 1。", "零點 x=−1，反曲點 (0,1)。"],
    solution: "x³ 往上抬 1：過 (−1,0) 與 (0,1)，處處遞增，(0,1) 是反曲點。" });
  sk("sk-008", 3, "x^3-3x", "x**3-3*x", [-3, 3, -4, 4], [[-2.1, 2.1]], {
    tags: ["extrema", "inflection"], hints: ["f′=3x²−3=0 → x=±1。", "f(−1)=2 極大、f(1)=−2 極小，零點 0、±√3。", "反曲點在原點。"],
    solution: "極大 (−1,2)、極小 (1,−2)、反曲點 (0,0)，零點 −√3、0、√3；兩端往 ∓∞。" });
  sk("sk-009", 3, "3x-x^3", "3*x-x**3", [-3, 3, -4, 4], [[-2.1, 2.1]], {
    tags: ["extrema", "inflection"], hints: ["是 x³−3x 的上下翻轉。", "極小 (−1,−2)、極大 (1,2)。"],
    solution: "x³−3x 上下翻轉：先降到 (−1,−2)，升到 (1,2)，再往下；原點是反曲點。" });
  sk("sk-010", 3, "x^3-3x^2", "x**3-3*x**2", [-2, 4, -5, 5], [[-1, 3.4]], {
    tags: ["extrema", "inflection"], hints: ["f=x²(x−3)：零點 0（重根）與 3。", "f′=3x(x−2)：極大 (0,0)、極小 (2,−4)。", "反曲點 x=1。"],
    solution: "極大在原點（碰 x 軸不穿過），降到 (2,−4)，再穿過 (3,0) 往上；反曲點 (1,−2)。" });
  sk("sk-011", 3, "x^4-2x^2", "x**4-2*x**2", [-2, 2, -2, 3], [[-1.7, 1.7]], {
    tags: ["extrema", "even-function"], hints: ["偶函數，對稱 y 軸。", "f′=4x(x²−1)：極小 (±1,−1)、極大 (0,0)。", "零點 0、±√2。"],
    solution: "W 形：兩個極小 (±1,−1)，原點是局部極大，零點 ±√2。" });
  sk("sk-012", 4, "(x-1)^2(x+2)", "(x-1)**2*(x+2)", [-3, 3, -6, 8], [[-2.4, 2.2]], {
    tags: ["extrema", "inflection"], hints: ["零點：−2（穿過）、1（重根，碰一下）。", "f′=3(x−1)(x+1)：極大 x=−1、極小 x=1。", "f(−1)=4，f(1)=0，f(0)=2。"],
    solution: "從左下穿過 (−2,0) 升到極大 (−1,4)，降到 (1,0) 碰 x 軸反彈往上；反曲點 (0,2)。" });

  /* ── B. 有理函數（R4）：垂直與水平漸近線，兩側分開畫 ── */
  sk("sk-013", 4, "\\frac{1}{x}", "1/x", [-4, 4, -4, 4], [[-4, -0.3], [0.3, 4]], {
    tags: ["rational", "asymptote"], hints: ["x=0 是垂直漸近線、y=0 是水平漸近線。", "第一、三象限各一支，都遞減。"],
    solution: "雙曲線：x>0 那支從右上角掉向 x 軸，x<0 那支在第三象限；兩支都遞減，不相連。" });
  sk("sk-014", 4, "\\frac{1}{x^2}", "1/x**2", [-4, 4, -1, 5], [[-4, -0.5], [0.5, 4]], {
    tags: ["rational", "asymptote", "even-function"], hints: ["偶函數，都在 x 軸上方。", "靠近 x=0 兩側都往 +∞。"],
    solution: "兩支都在上方，貼著 y 軸往上衝，往兩側貼向 x 軸。" });
  sk("sk-015", 4, "\\frac{1}{x-1}", "1/(x-1)", [-3, 4, -4, 4], [[-3, 0.6], [1.4, 4]], {
    tags: ["rational", "asymptote"], hints: ["1/x 右移 1。", "垂直漸近線 x=1，f(0)=−1。"],
    solution: "1/x 平移到 x=1：左支過 (0,−1) 往下衝，右支從上方掉向 x 軸。" });
  sk("sk-016", 4, "\\frac{x}{x-2}", "x/(x-2)", [-3, 6, -5, 6], [[-3, 1.4], [2.6, 6]], {
    tags: ["rational", "asymptote"], hints: ["f=1+2/(x−2)。", "垂直漸近線 x=2，水平漸近線 y=1，過原點。"],
    solution: "1+2/(x−2)：水平漸近線 y=1、垂直漸近線 x=2；左支從 y=1 下方降到 −∞（過原點），右支從 +∞ 降向 y=1。" });
  sk("sk-017", 4, "\\frac{x+1}{x-1}", "(x+1)/(x-1)", [-4, 5, -5, 6], [[-4, 0.5], [1.5, 5]], {
    tags: ["rational", "asymptote"], hints: ["f=1+2/(x−1)。", "零點 −1、f(0)=−1、漸近線 x=1 與 y=1。"],
    solution: "水平漸近線 y=1、垂直漸近線 x=1；左支過 (−1,0)、(0,−1) 往下，右支從上方降向 y=1。" });
  sk("sk-018", 4, "\\frac{x}{x^2+1}", "x/(x**2+1)", [-4, 4, -1, 1], [[-4, 4]], {
    tags: ["rational", "extrema", "odd-function"], hints: ["奇函數，過原點，水平漸近線 y=0。", "f′=(1−x²)/(x²+1)²：極大 (1,½)、極小 (−1,−½)。"],
    solution: "S 形波：從左邊貼 x 軸下方降到 (−1,−½)，穿過原點升到 (1,½)，再貼向 x 軸。" });
  sk("sk-019", 4, "\\frac{1}{x^2+1}", "1/(x**2+1)", [-4, 4, -0.5, 1.5], [[-4, 4]], {
    tags: ["rational", "extrema", "even-function"], hints: ["最大值 f(0)=1，偶函數。", "兩側貼向 y=0，反曲點 x=±1/√3。"],
    solution: "鐘形：頂點 (0,1)，兩側對稱下降貼向 x 軸，恆為正。" });
  sk("sk-020", 4, "\\frac{x^2}{x^2+1}", "x**2/(x**2+1)", [-4, 4, -0.5, 1.5], [[-4, 4]], {
    tags: ["rational", "even-function"], hints: ["f=1−1/(x²+1)。", "最小 f(0)=0，兩側升向 y=1。"],
    solution: "U 形底在原點，兩側對稱升向水平漸近線 y=1，永遠碰不到。" });
  sk("sk-021", 4, "x+\\frac{1}{x}", "x+1/x", [-4, 4, -5, 5], [[-4, -0.3], [0.3, 4]], {
    tags: ["rational", "asymptote", "extrema"], hints: ["斜漸近線 y=x、垂直漸近線 x=0。", "f′=1−1/x²：極小 (1,2)、極大 (−1,−2)。"],
    solution: "右支：從 +∞ 降到極小 (1,2) 再沿 y=x 上升；左支對稱：極大 (−1,−2)。" });
  sk("sk-022", 4, "x-\\frac{4}{x}", "x-4/x", [-5, 5, -8, 8], [[-5, -0.6], [0.6, 5]], {
    tags: ["rational", "asymptote"], hints: ["零點 ±2，斜漸近線 y=x。", "f′=1+4/x²>0：兩支都遞增。"],
    solution: "兩支都遞增：右支從 −∞ 升過 (2,0) 貼向 y=x，左支從 y=x 下方升過 (−2,0) 衝向 +∞。" });
  sk("sk-023", 5, "\\frac{x}{x^2-1}", "x/(x**2-1)", [-4, 4, -4, 4], [[-4, -1.3], [-0.7, 0.7], [1.3, 4]], {
    tags: ["rational", "asymptote", "odd-function"], hints: ["垂直漸近線 x=±1，水平漸近線 y=0，奇函數。", "f′=−(x²+1)/(x²−1)²<0：三段全都遞減。", "中間那段過原點。"],
    solution: "三支都遞減：左支從 x 軸下方掉向 −∞（x→−1⁻）；中間從 +∞ 降過原點到 −∞；右支從 +∞ 降回貼近 x 軸。" });

  /* ── C. 指數與對數（R3–R4） ── */
  sk("sk-024", 3, "e^{x}", "exp(x)", [-3, 2, -1, 6], [[-3, 1.7]], {
    tags: ["exponential"], hints: ["過 (0,1)，往左貼向 y=0。", "處處遞增、凹向上。"],
    solution: "往左貼向 x 軸，過 (0,1)，往右越來越陡。" });
  sk("sk-025", 3, "e^{-x}", "exp(-x)", [-2, 4, -1, 6], [[-1.7, 4]], {
    tags: ["exponential"], hints: ["eˣ 左右翻轉。", "遞減，過 (0,1)，往右貼向 y=0。"],
    solution: "從左上方掉下來，過 (0,1)，往右貼向 x 軸；凹向上。" });
  sk("sk-026", 3, "2^{x}", "2**x", [-3, 3, -1, 8], [[-3, 2.9]], {
    tags: ["exponential"], hints: ["過 (0,1)、(1,2)、(2,4)。", "跟 eˣ 同形，比較平。"],
    solution: "指數曲線：往左貼向 x 軸，過 (0,1)、(1,2)、(2,4)、(3,8)。" });
  sk("sk-027", 3, "\\ln x", "log(x)", [-1, 6, -3, 3], [[0.15, 6]], {
    tags: ["log"], hints: ["只在 x>0 有定義，x→0⁺ 掉向 −∞。", "過 (1,0)，遞增、凹向下。"],
    solution: "貼著 y 軸從 −∞ 升上來，穿過 (1,0)，之後越來越平地往上。" });
  sk("sk-028", 3, "\\ln(x^2+1)", "log(x**2+1)", [-3, 3, -1, 3], [[-3, 3]], {
    tags: ["log", "even-function"], hints: ["偶函數，最小 f(0)=0。", "反曲點 x=±1，之後凹向下。"],
    solution: "U 形底在原點，兩側對稱上升；|x|>1 後轉為凹向下、越來越平。" });
  sk("sk-029", 4, "xe^{-x}", "x*exp(-x)", [-1, 5, -1, 1], [[-0.5, 5]], {
    tags: ["exponential", "extrema"], hints: ["f′=(1−x)e⁻ˣ：極大 (1,1/e)。", "過原點，往右貼向 y=0；反曲點 x=2。"],
    solution: "從左下穿過原點升到極大 (1,0.37)，再緩慢下降貼向 x 軸。" });
  sk("sk-030", 4, "x^2e^{-x}", "x**2*exp(-x)", [-1, 6, -0.5, 1.5], [[-0.5, 6]], {
    tags: ["exponential", "extrema"], hints: ["f′=x(2−x)e⁻ˣ：極小 (0,0)、極大 (2,4/e²)。", "往右貼向 y=0。"],
    solution: "碰原點反彈，升到極大 (2,0.54)，再下降貼向 x 軸。" });
  sk("sk-031", 4, "e^{-x^2}", "exp(-(x**2))", [-3, 3, -0.5, 1.5], [[-3, 3]], {
    tags: ["exponential", "even-function", "inflection"], hints: ["鐘形，最大 (0,1)。", "反曲點 x=±1/√2。"],
    solution: "對稱的鐘形：頂點 (0,1)，兩側快速貼向 x 軸，恆為正。" });
  sk("sk-032", 4, "e^{x}-x", "exp(x)-x", [-2, 2, -1, 6], [[-2, 2]], {
    tags: ["exponential", "extrema"], hints: ["f′=eˣ−1=0 → x=0。", "極小 (0,1)；往左貼向 y=−x。"],
    solution: "極小 (0,1)，左邊沿 y=−x 上升，右邊指數上升；恆為正、凹向上。" });
  sk("sk-033", 4, "x\\ln x", "x*log(x)", [-0.5, 3, -1, 4], [[0.05, 3]], {
    tags: ["log", "extrema"], hints: ["x→0⁺ 時 f→0。", "f′=ln x+1=0 → x=1/e，極小 (1/e,−1/e)。", "過 (1,0)。"],
    solution: "從原點出發往下到極小 (0.37,−0.37)，回升穿過 (1,0) 後越來越陡。" });

  /* ── D. 三角與其他（R3–R4） ── */
  sk("sk-034", 3, "\\sin x", "sin(x)", [-7, 7, -2, 2], [[-6.5, 6.5]], {
    tags: ["trig"], hints: ["過原點往上，週期 2π。", "極大在 π/2、極小在 3π/2。"],
    solution: "波：零點 0、±π、±2π，極大 π/2+2kπ、極小 −π/2+2kπ。" });
  sk("sk-035", 3, "\\cos x", "cos(x)", [-7, 7, -2, 2], [[-6.5, 6.5]], {
    tags: ["trig", "even-function"], hints: ["從 (0,1) 出發，偶函數。", "零點 ±π/2、±3π/2。"],
    solution: "sin 左移 π/2：頂點 (0,1)，零點 ±π/2，極小 ±π。" });
  sk("sk-036", 4, "\\sin(2x)", "sin(2*x)", [-4, 4, -2, 2], [[-3.5, 3.5]], {
    tags: ["trig"], hints: ["週期變成 π。", "零點 0、±π/2、±π；極大在 π/4。"],
    solution: "sin 壓縮一半：每 π 一個完整波，零點在 kπ/2。" });
  sk("sk-037", 4, "\\tan x", "tan(x)", [-4.5, 4.5, -4, 4], [[-4.4, -1.85], [-1.3, 1.3], [1.85, 4.4]], {
    tags: ["trig", "asymptote"], hints: ["垂直漸近線 x=±π/2、±3π/2。", "每一支都遞增，過 kπ 的零點。"],
    solution: "三支都遞增，各自從 −∞ 升到 +∞；零點 −π、0、π；漸近線在 ±π/2、±3π/2。" });
  sk("sk-038", 4, "\\arctan x", "atan(x)", [-6, 6, -2, 2], [[-6, 6]], {
    tags: ["trig", "odd-function"], hints: ["遞增，水平漸近線 y=±π/2。", "過原點，原點是反曲點。"],
    solution: "S 形：從 y=−π/2 升過原點到 y=π/2，左凹上右凹下。" });
  sk("sk-039", 3, "\\sqrt{x}", "sqrt(x)", [-1, 7, -1, 4], [[0, 6]], {
    tags: ["radical"], hints: ["只在 x≥0，從原點出發。", "遞增、凹向下，過 (1,1)、(4,2)。"],
    solution: "從原點往右上方，越來越平：過 (1,1)、(4,2)。" });
  sk("sk-040", 3, "\\sqrt{4-x^2}", "sqrt(4-x**2)", [-3, 3, -1, 3], [[-2, 2]], {
    tags: ["radical", "even-function"], hints: ["y²=4−x² → x²+y²=4 的上半。", "半徑 2 的上半圓。"],
    solution: "上半圓：從 (−2,0) 經 (0,2) 到 (2,0)。" });
  sk("sk-041", 4, "x^2-|x|", "x**2-abs(x)", [-3, 3, -1, 4], [[-2, 2]], {
    tags: ["absolute-value", "extrema"], hints: ["偶函數；x≥0 時是 x²−x。", "極小 (±½,−¼)，原點是尖點（局部極大）。", "零點 0、±1。"],
    solution: "W 形但中間是尖角：極小 (±½,−¼)，原點尖點，穿過 ±1 往上。" });
  sk("sk-042", 3, "|x^2-1|", "abs(x**2-1)", [-3, 3, -1, 4], [[-2.2, 2.2]], {
    tags: ["absolute-value", "extrema"], hints: ["x²−1 在 x 軸下方那段翻上來。", "尖點 (±1,0)，局部極大 (0,1)。"],
    solution: "拋物線把 |x|<1 那段翻成向下的弧：尖點 ±1，中間頂點 (0,1)。" });
