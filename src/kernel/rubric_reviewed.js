...
(function () {
  ...
  const REVIEWED = {
    ...
    // ── 2026-09-08 新手池審計 ────────────────────────────────
    // 反推的三軸把一批 IBP／部分分式／瑕積分題排進 R2，cap=2 的新手
    // 在快速訓練就會遇到還沒學的技巧。原則：用到「要先學過才會」的
    // 工具（分部、拆分式、瑕積分收斂、偏微分符號）至少 R3；
    // 兩個技巧串接或有隱藏一步的至少 R4。
    "int-028": { axes: [2, 1, 2], why: "∫log x dx 要想到「乘 1 再分部」—— 沒學過分部的人連下手點都沒有。" },
    "int-038": { axes: [2, 1, 2], why: "∫x cos x dx：標準一次分部。技巧本身就是門檻，R3 起跳。" },
    "int-052": { axes: [2, 1, 2], why: "∫x²eˣ dx 要分部兩次並追蹤符號，是分部的第二課，不是新手題。" },
    "int-053": { axes: [2, 1, 2], why: "∫x²sin x dx：同 int-052，兩次分部。" },
    "int-055": { axes: [2, 1, 2], why: "∫(log x)² dx：乘 1 分部之後又回到 ∫log x，要再分部一次。" },
    "int-056": { axes: [2, 1, 2], why: "∫₀¹x arctan x dx：分部後還要處理 ∫x²/(1+x²)（拆成 1−1/(1+x²)）。" },
    "exam-int-004": { axes: [2, 1, 2], why: "∫x log x dx：標準一次分部。" },
    "exam-int-013": { axes: [2, 1, 2], why: "∫arctan x dx：乘 1 分部＋後段 u-sub，兩小步。" },
    "exam-int-029": { axes: [2, 1, 2], why: "∫₀^π x sin x dx：一次分部加代界。" },
    "uni-int-003": { axes: [2, 1, 2], why: "∫log x/x² dx：分部方向要選對（log 微分、x⁻² 積分）。" },
    "uni-int-011": { axes: [2, 1, 2], why: "∫x²log x dx：標準一次分部。" },
    "uni-int-012": { axes: [2, 1, 2], why: "∫x arctan x dx：分部＋1/(1+x²) 的整理。" },
    "uni-int-015": { axes: [2, 1, 2], why: "∫₀¹x log(1+x) dx：分部後的 ∫x²/(1+x) 還要做多項式除法。" },
    "depth-int-016": { axes: [2, 1, 2], why: "∫₀^{π/2}x cos x dx：一次分部。掛著 depth 標籤但正確難度是 R3。" },
    "burst-int-039": { axes: [2, 1, 2], why: "∫x⁴log x dx：一次分部。R2 會讓還沒學分部的人在快速訓練撞牆。" },
    "world-050": { axes: [2, 1, 2], why: "∫₀¹x³log x dx：分部＋x→0⁺ 端點的極限要交代。" },
    "hc-ibp-007": { axes: [2, 1, 2], why: "∫x³log x dx：一次分部。" },
    "hc-ibp-009": { axes: [2, 1, 2], why: "∫₀^{π/2}x²cos x dx：分部兩次。" },
    "cx-exam-006": { axes: [2, 1, 2], why: "∫₀¹x²eˣ dx：分部兩次，經典段考題，R3。" },
    "burst-boss2-int-013": { axes: [2, 1, 2], why: "∫₀^{π/2}x sin x cos x dx：先倍角化成 ½∫x sin 2x 再分部 —— 有一步辨識。" },
    "exam-int-012": { axes: [2, 2, 1], why: "∫e^{2x}sin x dx：循環分部，要認出「積回自己」再解方程 —— 這個結構沒看過就不會。" },
    "uni-int-010": { axes: [2, 2, 1], why: "∫eˣcos x dx：同上，循環分部。" },
    "uni-int-004": { axes: [2, 2, 2], why: "∫arctan x/x² dx：分部之後的 ∫1/(x(1+x²)) 還要拆分式，兩技巧串接。" },
    "hc-ibp-008": { axes: [2, 2, 2], why: "∫₀¹x⁴log(1+x) dx：分部後是五次多項式除法加端點，量大且雜。" },
    "hc-ibp-010": { axes: [2, 2, 2], why: "∫₀¹x³arctan x dx：分部後 ∫x⁴/(1+x²) 要除法再 arctan，兩層。" },
    "burst-int-040": { axes: [2, 2, 2], why: "∫x³arctan x dx：同 hc-ibp-010。" },
    "world-018": { axes: [2, 2, 2], why: "∫₀¹log x/(1+x)² dx：分部後接 ∫1/(x(1+x))，且 x→0⁺ 是 0·∞ 端點 —— 隱藏兩步。" },
    "world-082": { axes: [2, 2, 2], why: "∫₀¹(arcsin x)² dx：分部兩次夾一個三角代換，串接題。" },
    "world-092": { axes: [2, 2, 2], why: "∫₀¹x log(1−x) dx：端點 x→1⁻ 發散要用極限收拾，不只是機械分部。" },
    "world-095": { axes: [2, 2, 2], why: "∫₀^{π/2}sin x log(sin x) dx：u=cos x 之後還要 log 的分部，兩技巧。" },
    "gap-int-pf-001": { axes: [2, 1, 2], why: "∫1/(x²−1) dx：要會拆 A/(x−1)+B/(x+1)。部分分式是教了才會的技巧，R3 起跳。" },
    "gap-int-pf-002": { axes: [2, 1, 2], why: "∫(x+1)/(x²+x) dx：先因式分解再拆分式。" },
    "gap-int-pf-003": { axes: [2, 1, 2], why: "∫1/(x(x+2)) dx：標準兩項拆分。" },
    "gap-int-pf-004": { axes: [2, 1, 2], why: "∫(2x+3)/(x²+3x+2) dx：分母因式分解＋拆分。" },
    "gap-int-pf-006": { axes: [2, 1, 2], why: "∫(3x+5)/(x²+x−2) dx：同上。" },
    "gap-int-pf-007": { axes: [2, 1, 2], why: "∫(x+4)/(x²+5x+6) dx：同上。" },
    "cx-pf-001": { axes: [2, 1, 2], why: "∫₀¹dx/((x+1)(x+2))：拆分式加代界。" },
    "cx-pf-002": { axes: [2, 1, 2], why: "∫₂³dx/(x²−1)：拆分式加代界。" },
    "cx-pf-003": { axes: [2, 1, 2], why: "∫₀¹x dx/((x+1)(x+2))：分子帶 x 的拆分，係數解起來多一步。" },
    "tmpl-int-partialfrac-001": { axes: [2, 1, 2], why: "∫1/(x(x+1)) dx：部分分式模板題，技巧門檻同 gap-int-pf-003。" },
    "tmpl-int-partialfrac-002": { axes: [2, 1, 2], why: "同 family：∫1/(x(x+2))。" },
    "tmpl-int-partialfrac-003": { axes: [2, 1, 2], why: "同 family：∫1/(x(x+1)) 換界。" },
    "tmpl-int-partialfrac-004": { axes: [2, 1, 2], why: "同 family：∫1/(x(x+3))。" },
    "int-064": { axes: [2, 2, 2], why: "∫₀^π x sin x/(1+cos²x) dx：King's property 的招牌題，x→π−x 這步沒看過就一定卡。" },
    "uni-int-025": { axes: [2, 1, 2], why: "∫₀^∞1/(x²+4) dx：積分本身是 arctan，但「上限是 ∞」的收斂概念是新的一課。" },
    "depth-int-019": { axes: [2, 1, 2], why: "∫₀^∞x/(1+x²)² dx：u-sub＋瑕積分取極限。" },
    "burst-boss-int-005": { axes: [2, 2, 2], why: "∫₀^∞x²/(1+x⁶) dx：要看出 u=x³ 化成 arctan，加上瑕積分 —— true-boss 標籤掛在 R2 是反推的事故。" },
    "burst-boss-int-007": { axes: [2, 2, 2], why: "∫₀^∞x/(1+x⁴) dx：同上，u=x²。" },
    "depth-lim-008": { axes: [2, 2, 2], why: "(arcsin x−arctan x)/x³：兩個反三角都要展到 x³，同 uni-lim-010（已複核 R4）。" },
    "depth-der-004": { axes: [2, 1, 2], why: "d³/dx³(x²log x)：連微三次，每一次都要乘法法則，記帳量就是難度。" },
    "depth-der-006": { axes: [2, 1, 2], why: "d/dx log(x+√(x²+4))：鏈鎖之後要看出化簡成 1/√(x²+4)。" },
    "depth-int-002": { axes: [2, 2, 2], why: "∫x⁵/(1+x³) dx：先多項式除法再處理 1/(1+x³) 的三次因式分解，兩層。" },
    "depth-int-011": { axes: [2, 2, 1], why: "∫x²/(1+x²)² dx：要拆成 1/(1+x²)−1/(1+x²)²，第二項還要降冪公式。" },
    "depth-int-015": { axes: [2, 1, 2], why: "∫₀¹x/(1+x+x²) dx：配方＋log 與 arctan 兩塊拼起來。" },
    "depth-int-018": { axes: [2, 1, 2], why: "∫₀¹x³/(1+x²)² dx：u=x² 後還要拆成兩項。" },
    "burst-boss2-anti-006": { axes: [2, 1, 2], why: "∫x⁵/(1+x²) dx：多項式除法（三步商）再接 log。" },
    "burst-boss3-anti-006": { axes: [2, 1, 2], why: "∫x⁷/(1+x²) dx：同上，除法更長。" },
    "der-025": { axes: [2, 1, 2], why: "∂/∂x log(x²+y²)：計算是鏈鎖，但「把 y 當常數」的偏微分規則是還沒教就看不懂的符號。" },
    "der-033": { axes: [2, 1, 2], why: "∂²/∂x²(x²y+sin x)：同上，偏微分符號本身是門檻。" }
  };
