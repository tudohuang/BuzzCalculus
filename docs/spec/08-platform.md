...
**這是跨科最強的價值**，而且技術上只需要一個共用的 records 存取層。
...
- **階段 0（已完成，2026-08-17）：先讓它好寫。**
  這一階原本不在規格裡，是實際拿 iPad 用之後補上的 —— 而它其實是所有後面階段的前提：
  沒有人會在一塊不好寫的板子上留下值得回放的筆跡。

  四個瀏覽器預設行為會直接毀掉 Apple Pencil 的體驗，而且全部是行為層的問題，
  字串比對一條都抓不到：

  | 問題 | 現象 | 處理 |
  | --- | --- | --- |
  | 長按選取 | 手掌或筆尖碰到旁邊就整段反白、跳放大鏡與拷貝選單 | 全站 `user-select: none` + `-webkit-touch-callout: none`，只有輸入框與法務頁例外 |
  | 手掌 | 手掌是 `pointerType:"touch"`，跟筆同時發生，拖出一條橫貫全頁的線 | 看到筆之後 700ms 內的觸控一律忽略 |
  | 取樣不足 | `pointermove` 一 frame 只觸發一次，Pencil 取樣 120–240Hz | `getCoalescedEvents()` |
  | 重畫成本 | 每次 move 重畫整塊板是 O(n²)，寫到第三行開始頓 | `stroke.drawnTo` 只畫新增的那一段 |

  另外兩個「感覺不靈敏」的來源，都是實作細節而不是硬體限制：

  - **筆跡落後筆尖**：二次曲線要知道下一個取樣點才畫得出來，所以已提交的筆跡
    永遠停在倒數第二個點。原本只有收筆才補到真正的筆尖 —— 寫得越慢越明顯，
    而算數學的時候人本來就寫得慢。改成每次 move 都補到筆尖，代價是輕微重畫。
  - **落筆瞬間太細**：Pencil 剛碰到玻璃時壓力接近 0，照原始值畫出來是一條髮絲。
    使用者的感覺不是「這裡比較細」，是「筆沒反應」。壓力墊到 0.35 再算寬度。

  繪製核心在 `src/kernel/board_render.js`（純函式），行為由
  `validate_golden.js` 的 `kernel.board` 釘住 —— 它用假的 ctx 錄下繪圖指令序列，
  改筆寬或壓感曲線就會紅。書寫行為由 `tools/e2e_handwriting.js` 驗，
  斷言全部打在**畫布的像素**上。
