...
| 項目 | 數字 | 備註 |
| --- | ---: | --- |
| 題目總數 | 2423 | 純微積分；理科秒殺包已於 2026-08 移出 |
| 答案通過獨立數值驗算 | 2144（88%） | 其餘是證明題與定性題，本質上沒有可比對的數值 |
| 有 `source` | 2343 |  |
| 有 `tags` | 2423 | distinct tag 364 個，其中 283 個是技巧 tag |
| 有 `solution` | 2423 | 單段文字 |
| 有作者撰寫 `hints` | 1813 |  |
| 有 `solutionSteps` | 260 | 結構化步驟，仍是最大的內容缺口 |
| 證明題 | 68 | 含 Lean 機器驗證 8 則 |
| `src/app.js` | 15491 行 | 單一 IIFE，拆分進行中 |
| `styles.css` | 12081 行 |  |
| 題庫檔 `src/problem_*.js` | 36 |  |
| kernel 模組 `src/kernel/*.js` | 22 | 純函式層 |
| CI 驗證器 `tools/` | 43 支 | validate / verify / smoke / e2e |

**答案型別分佈**：`numeric` 1609、`expression` 346、`antiderivative` 142、`text` 115、`sketch` 42、`worksheet` 42、`graphtap` 38、`graph` 26、`interval` 20、`set` 17、`proof` 16、`graphslope` 10。

**難度分佈**：R1 182 / R2 418 / R3 627 / R4 735 / R5 352 / R6 109。
