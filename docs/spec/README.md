...
| 項目 | 數字 | 備註 |
| --- | ---: | --- |
| 題目總數 | 1994 | 純微積分；理科秒殺包已於 2026-08 移出 |
| 答案通過獨立數值驗算 | 1753（88%） | 其餘是證明題與定性題，本質上沒有可比對的數值 |
| 有 `source` | 1914 |  |
| 有 `tags` | 1994 | distinct tag 324 個，其中 256 個是技巧 tag |
| 有 `solution` | 1994 | 單段文字 |
| 有作者撰寫 `hints` | 1620 |  |
| 有 `solutionSteps` | 226 | 結構化步驟，仍是最大的內容缺口 |
| 證明題 | 41 | 含 Lean 機器驗證 8 則 |
| `src/app.js` | 15371 行 | 單一 IIFE，拆分進行中 |
| `styles.css` | 11923 行 |  |
| 題庫檔 `src/problem_*.js` | 35 |  |
| kernel 模組 `src/kernel/*.js` | 21 | 純函式層 |
| CI 驗證器 `tools/` | 36 支 | validate / verify / smoke / e2e |
...
**難度分佈**：R1 182 / R2 386 / R3 512 / R4 563 / R5 255 / R6 96。
