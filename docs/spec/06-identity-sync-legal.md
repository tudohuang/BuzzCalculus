...
**舊 LWW 的致命問題**：手機練了 10 題、電腦練了 5 題，同步後其中一邊整份消失。
對「訓練紀錄」這種只增不減的資料，這是不可接受的。

> **狀態（2026-08）：已經改掉了。** `syncNow()` 一度還在用整包 LWW ——
> 也就是說 merge 早就寫好、也已經用在「匯入 JSON」上，卻沒有接到同步這條路徑。
> 那個落差很容易被漏掉，因為單一裝置上永遠看不出問題。
> 現在 pull 之後一律走 `BuzzRecords.merge()`，合併結果先寫回本機再推上去
> （只推不存的話兩邊不會收斂）。**缺的只剩伺服器那一端。**
...
驗證在 `tools/validate_records_v2.js`（不是另開一支測試檔）：
