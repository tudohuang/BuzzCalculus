...
期別代號：`P0` 固化 / `P1` 收納 / `P2` 診斷 / `P3` 內容 / `P3.5` 產品化 / `P4` 帳號 /
`P5` 社群 / `P6` 平台 / `✓` 現況已具備 / `∞` 持續性原則（不是可勾選的工作）

## 期別完成狀態（2026-08-17）

這張表是**驗收時**讀的，所以要先知道哪幾期已經關掉。逐條的勾選在
[09-roadmap.md](09-roadmap.md)，這裡只記期別層級。
...
| P1 收納 | ✅ | — |
| P2 診斷 | ✅ | — |
| P3 內容 | 🔶 大部分完成 | 結構化解題步驟（81/1459）、525 題推不出可驗證的第二層提示、187 題降級待複核 |
| P3.5 產品化 | 🔶 19/20 | 無障礙走查（局部渲染已開路：動作委派 + 計算紙局部更新，但 render() 仍是整頁） |
| P4 帳號 | 🔶 部分 | **只缺伺服器那一端**：合併、法遵、隱私開關、回報出口都已上線 |
| P5 社群 | ⬜ | 全部 |
| P6 平台 | ⬜ | 全部 |

**目前最大的三個缺口**，照「擋住什麼」排序：

1. **結構化解題步驟**（P3）—— 1459 題裡只有 81 題有。這是內容缺口，不是機制缺口；
   機制早就完成，缺的是人寫。它同時擋住 [08.5 階段 3](08-platform.md#85-手寫辨識與筆跡回放)
   （要對齊筆跡與解題步驟才找得出「出錯的那一行」）。
2. **後端**（P4）—— 不擋任何現有功能（匯出／匯入就是完整的解法），
   但擋住跨裝置自動同步與實證難度校準。這是**產品決策**不是技術債。
3. **局部渲染**（P3.5）—— 擋住手感的上限。目前是止血。
...
這些不是從需求清單推出來的，是**實際拿去用**之後才發現非做不可的。
記在這裡是為了讓下一次重讀 spec 的人知道它們的來歷 —— 需求清單不是全知的。

| 項目 | 來源 | 規格 |
| --- | --- | --- |
| iPad 手寫的輸入層（防手掌、取樣、延遲、壓感） | 拿 iPad 實際寫過 | [08.5 階段 0](08-platform.md#85-手寫辨識與筆跡回放) |
| 全站關閉長按選取 | 同上 —— 用筆算式時手掌一碰旁邊就整段反白 | 同上 |
| 出卷（照條件抽一份可列印的卷） | 家教與助教的實際工作流程 | [09 P3.5](09-roadmap.md#p35--產品化2026-08-17-完成-1820) |
| 產品化的其餘 17 項 | 「內容品質遠超前外觀給人的印象」 | 同上 |
| Spec 現況快照自動產生 | 快照本身過期了，而每一節都寫著「必須相容於此」 | `tools/spec_snapshot.js` |

**移除的**（原本存在、現在確定不做）：

| 項目 | 為什麼移除 |
| --- | --- |
| 全螢幕鎖定與切頁判錯 | 立場是「假設使用者會作弊」，但這是自己練給自己看的工具。防不了任何東西，卻會誤傷接電話、被通知蓋掉、想開計算機的人。見 [00 P4](00-north-star.md#p4-焦慮只准存在於考試模式) |
| 切頁次數的遙測 | 那個數字唯一的用途是判他答錯。判錯拿掉之後就只剩監控，而隱私政策寫的是「只送哪個功能被用了幾次」 |
