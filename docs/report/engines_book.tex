...
\newtcolorbox{example}[1][例]{enhanced,breakable,colback=paper,colframe=line,boxrule=0.5pt,arc=2pt,
...
$\lim$ 有太多寫法：\code{lim\_\{x→2\} 3x = 6}、\code{lim x→2 (3x) = 6}、\code{lim\_(x->2) 3x=6}。\code{canonicalText} 把它們收斂成 \code{lim[x->2]3x=6}：抓 \code{lim}、可有可無的底線與括號、變數、箭頭、目標值，重組成固定格式，去乘號，再把緊接的一層括號拆掉。目標比對用這個鍵。
