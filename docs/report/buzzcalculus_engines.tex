...
\newcommand{\screen}[2]{\begin{center}\fbox{\includegraphics[width=#1]{#2}}\end{center}}
...
\begin{figure}[h]
\centering
\fbox{\includegraphics[width=0.48\textwidth]{onboarding}}\hfill
\fbox{\includegraphics[width=0.48\textwidth]{home}}
\caption{第一次進站只問一件事（左）；練過之後的首頁只有一顆主按鈕，四種練法收進「換一種練法」（右）。}
\end{figure}
...
\begin{tikzpicture}[x=1.55cm]
  ...
  \node[above=5pt,font=\scriptsize\color{rust}] at (1,0.2) {避開的「漂亮數」};
  \node[above=5pt,font=\scriptsize\color{gold}] at (-1.3,0.2) {負數：分開 $\sqrt{x^2}$ 與 $x$};
\end{tikzpicture}
\caption{單變數的八個取樣點（金點）。紅線是刻意避開的 0.5、1、2：不同函數在那裡撞在一起的機率最高。}
\end{figure}
...
\begin{figure}[h]
\centering
\fbox{\includegraphics[width=0.30\textwidth]{phone-quiz}}\hfill
\fbox{\includegraphics[width=0.66\textwidth]{results}}
\caption{手機上的作答畫面（左）與結算頁（右）。結算只有一顆主按鈕，其餘降成文字連結；四個數字不做動畫——成績是事實，不是表演。}
\end{figure}
...
\begin{tikzpicture}[
  ...
  box/.style={draw=line,fill=paper,rounded corners=2pt,minimum height=11mm,text width=34mm,align=center,font=\sffamily\small},
  hot/.style={box,draw=gold,line width=1pt},
  arr/.style={-{Latex[length=2mm]},draw=muted,line width=0.8pt}
]
\node[box] (t) {一行一句的證明\\{\scriptsize\color{muted}全形、希臘字母、LaTeX 先正規化}};
\node[hot,right=of t] (a) {句型引擎\\{\scriptsize\color{muted}13 種句型}};
\node[hot,right=of a] (b) {代數引擎\\{\scriptsize\color{muted}假設下隨機取點}};
\node[hot,right=of b] (c) {推論引擎\\{\scriptsize\color{muted}規則字典}};
\node[box,below=of b] (r) {逐行三色 + 骨架 + 反例\\{\scriptsize\color{muted}\textcolor{moss}{綠} 驗過成立 · \textcolor{rust}{紅} 驗過不成立 · \textcolor{gold}{黃} 驗不了}};
\draw[arr] (t) -- (a); \draw[arr] (a) -- (b); \draw[arr] (b) -- (c);
\draw[arr] (a.south) |- (r.west); \draw[arr] (b) -- (r); \draw[arr] (c.south) |- (r.east);
\end{tikzpicture}
}
\caption{白話證明的三個引擎與它們共同產出的報告。}
\end{figure}
...
\begin{figure}[h]
\centering
\fbox{\includegraphics[width=\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax]{proof-editor}}
\caption{題目編輯器：左題右寫，打字即時逐行判定。這份 $\delta = \varepsilon$ 的證明紅在第 4 行，結論標黃。}
\end{figure}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \fill[mosssoft] (0,2.2) rectangle (4.4,4.4); \fill[goldsoft] (0,0) rectangle (4.4,2.2);
  \fill[rustsoft] (4.4,2.2) rectangle (8.8,4.4); \fill[paper] (4.4,0) rectangle (8.8,2.2);
  \draw[line] (0,0) rectangle (8.8,4.4); \draw[line] (4.4,0) -- (4.4,4.4); \draw[line] (0,2.2) -- (8.8,2.2);
  \node[font=\sffamily\bfseries] at (2.2,3.6) {反射區}; \node[font=\scriptsize\color{muted},text width=38mm,align=center] at (2.2,2.9) {快而準：拉高難度或進 Boss};
  \node[font=\sffamily\bfseries] at (6.6,3.6) {衝太快}; \node[font=\scriptsize\color{muted},text width=38mm,align=center] at (6.6,2.9) {快但不準：練習模式＋標註錯因};
  \node[font=\sffamily\bfseries] at (2.2,1.4) {會但慢}; \node[font=\scriptsize\color{muted},text width=38mm,align=center] at (2.2,0.7) {準但慢：同技巧限時，時限逐次縮短};
  \node[font=\sffamily\bfseries] at (6.6,1.4) {還沒建立}; \node[font=\scriptsize\color{muted},text width=38mm,align=center] at (6.6,0.7) {關鍵一句＋三層提示＋慢練};
  \node[font=\scriptsize\color{muted},rotate=90] at (-0.4,2.2) {快 $\leftarrow$ elapsed / timeLimit $\rightarrow$ 慢};
  \node[font=\scriptsize\color{muted}] at (4.4,-0.4) {準（正確率 $> 0.7$）$\leftarrow$ \quad $\rightarrow$ 不準};
\end{tikzpicture}
\caption{速度象限。同樣 60\% 的正確率，落在不同格子的處方完全不同。}
\end{figure}
...
\begin{figure}[h]
\centering
\fbox{\includegraphics[width=\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax]{insights}}
\caption{數據頁：技巧掌握度、速度象限與錯因統計。樣本不夠的技巧留白，不給不準的數字。}
\end{figure}
...
\begin{figure}[h]
\centering
\fbox{\includegraphics[width=0.48\textwidth]{course-index}}\hfill
\fbox{\includegraphics[width=0.48\textwidth]{course-lesson}}
\caption{課程表（左）與單課頁（右）：概念、逐步示範（揭到第二步）、小測、練習。}
\end{figure}
...
\begin{figure}[h]
\centering
\fbox{\includegraphics[width=\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax]{quiz-board}}
\caption{自己寫模式的作答畫面，計算紙攤開並用合成的筆事件寫了幾筆：壓感讓線條粗細變化，落筆與收筆不會收成針尖。}
\end{figure}
...
\centering
\begin{tikzpicture}[x=1cm,y=1cm]
  \draw[line width=0.8pt,color=muted,-{Latex[length=2mm]}] (0,0) -- (12.4,0);
  \node[font=\scriptsize\color{muted}] at (12.2,-0.35) {時間 $\rightarrow$};
  \fill[goldsoft] (9.4,0.3) rectangle (12,0.9); \node[font=\sffamily\small] at (10.7,0.6) {history · 40 場};
  \fill[mosssoft] (4.8,1.2) rectangle (12,1.8); \node[font=\sffamily\small] at (8.4,1.5) {attemptLog · 5,000 筆 tuple · 約半年};
  \fill[rustsoft] (0.4,2.1) rectangle (12,2.7); \node[font=\sffamily\small] at (6.2,2.4) {sessions · 400 場摘要 · 一年以上};
  \node[font=\scriptsize\color{muted},anchor=west] at (0.4,3.2) {同一把 localStorage key；舊使用者第一次載入從 history 回填，不需要遷移腳本};
\end{tikzpicture}
\caption{三層紀錄各自負責的時間尺度。能力模型讀中間那層。}
\end{figure}
...
\begin{figure}[h]
\centering
\fbox{\includegraphics[width=0.34\textwidth]{phone-home}}\hfill
\fbox{\includegraphics[width=0.62\textwidth]{proof-index}}
\caption{手機首頁（左）與證明訓練的題目表（右）：兩個題庫在同一張表，進度、難度、狀態一眼看完。}
\end{figure}
...
\begin{tikzpicture}[
  ...
  box/.style={draw=line,fill=paper,rounded corners=2pt,minimum height=9mm,text width=24mm,align=center,font=\sffamily\footnotesize},
  bad/.style={box,draw=rust},
  arr/.style={-{Latex[length=2mm]},draw=muted,line width=0.8pt}
]
\node[box] (a) {typecheck};
\node[box,right=of a] (b) {41 支驗證器\\{\scriptsize ci\_checks.js}};
\node[box,right=of b] (c) {9 支 E2E\\{\scriptsize 真的 Chrome}};
\node[box,right=of c] (d) {獨立驗算\\{\scriptsize 1,753 題}};
\node[box,right=of d] (e) {Lean\\{\scriptsize 8 題}};
\node[box,right=of e] (f) {部署};
\draw[arr] (a)--(b); \draw[arr] (b)--(c); \draw[arr] (c)--(d); \draw[arr] (d)--(e); \draw[arr] (e)--(f);
\node[bad,below=of c] (x) {任何一條紅\\{\scriptsize ::error annotation}};
\draw[arr,draw=rust] (b.south) |- (x.west); \draw[arr,draw=rust] (d.south) |- (x.east);
\node[font=\scriptsize\color{muted},below=1mm of x] {站台不更新；annotation 走公開 API 就讀得到};
\end{tikzpicture}
}
\caption{部署與測試在同一個 job：任何一條斷言紅，站台就不更新。}
\end{figure}
